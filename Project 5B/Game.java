...
public class Game {
   ...
   private void initializeGame() {
      ...
      // Cards get dealt out to each player in round-robin fashion, i.e.,
      ...
      for (int i = 0; i < handSize; i++) {
         for (int j = 0; j < players.length; j++) {
            players[j].hand.cards[i] = drawPile.discard();
            players[j].hand.numFilled++;
         }
      }
   ...
   private Player nextPlayer() {
      ...
      if (discardPile.top().getRank() == 12) {
         displaySkippedPlayer(players[(current + 1) % 
                            players.length].getName());
         waitForUser();
         displayGameState();
         current = (current + 2) % players.length;
         return players[(current) % players.length];
      }
      else {
  ...
  /**
   ...
   public static boolean cardMatches(Card card1, Card card2) {
      ...
      if (card1.getRank() == 8 || card2.getSuit() == 8) {
         return true;
      }
      else {
   ...
   private boolean gameOver() {
      ...
      return players[current].hand.size() == 0;

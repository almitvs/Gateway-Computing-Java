...
public class User extends Player {
   ...
   public Card makeMove(Game crazyEight, Card top) {
      ...
      input.nextLine();
      if (n == 0) {
         if (this.hasPlayableCard(top)) {
            System.out.print(NO_DRAW_IF_HAVE_PLAYABLE_CARD_MESSAGE);
            return makeMove(crazyEight, top);
         }
         else {
            hand.addCard(crazyEight.draw());
            System.out.print("*** " + name + " draws " + 
               this.hand.getCard(this.hand.size() - 1) + "\n");
            System.out.println(this.hand);
            return makeMove(crazyEight, top);
         }
      }
      else if (n > 0 && n <= this.hand.size()) {
         if (Game.cardMatches(this.hand.cards[n - 1], top)) {
            return this.hand.discard(n - 1);
         }
         else {
